...
2. Adapt or fix generated code until it runs cleanly. "Adapt or fix" is limited to runtime/environment fixes (bugs, OOM, imports, config); do not change model architecture, hyperparameters, or training logic to improve metrics.
...
## Constraints
- Do not change model architecture, optimizer, or training logic to improve metrics; only make the existing plan run (fix bugs, OOM via batch size, etc.).
